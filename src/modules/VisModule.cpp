...
void VisModule::renderPositionTrace(GLuint shaderProgramId, uint64_t simulationTime) {
    ...
    for (StampedPosition& pos : tracedPositions) {
        ...
        float r = 0.5 + 0.5 * std::sin(
                (float)(simulationTime - pos.time) * 0.01);
        float g = 0.5 + 0.5 * std::sin(
                (float)(simulationTime - pos.time) * 0.01 + glm::radians(120.0f));
        float b = 0.5 + 0.5 * std::sin(
                (float)(simulationTime - pos.time) * 0.01 + glm::radians(240.0f));
