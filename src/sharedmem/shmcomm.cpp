...
void *SHMCommPrivate::_lock(LockMode mode)
{
    ...
    if(mode == WRITE_OVERWRITE_OLDEST || mode == READ_OLDEST)bestId = -1;
    ...
    if(mode == WRITE_NO_OVERWRITE)return nullptr;
    else if(bestBuf == nullptr)return nullptr;
...
{
    if(shmPtr != nullptr)detach();
    if(role == SERVER)destroy();
}
